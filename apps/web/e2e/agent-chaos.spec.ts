...
type ServerMessage =
  ...
  | { type: "TOOL_CALL"; seq: number; stream_id: string; call_id: string; tool_name: string; args: Record<string, unknown> }
  | { type: "TOOL_RESULT"; seq: number; stream_id: string; call_id: string; result: Record<string, unknown> }
...
async function installControlledWorkerWebSocket(page: Page) {
  ...
  await page.route("**/*", async (route) => {
    ...
    await route.fulfill({ response, body: `${controlledWebSocketShim()}\n${body}` });
  });
}
...
(() => {
  ...
  class ControlledWebSocket {
    ...
    constructor(url) {
      ...
      sockets.push(this);
      self.postMessage({ kind: "__test_socket_opened", socketIndex: this.socketIndex, url });
      queueMicrotask(() => this.__open());
    }
    close() { this.readyState = ControlledWebSocket.CLOSED; }
    send(payload) {
      const parsed = JSON.parse(payload);
      this.sent.push(parsed);
      self.postMessage({ kind: "__test_socket_sent", socketIndex: this.socketIndex, payload: parsed });
    }
    __open() {
...
async function receive(page: Page, message: ServerMessage, socketIndex?: number) {
  await page.evaluate(
    ({ message, socketIndex }) => window.__agentTest!.postToWorker({ op: "receive", message, socketIndex }),
    { message, socketIndex },
  );
}
...
async function receiveRaw(page: Page, data: string, socketIndex?: number) {
  await page.evaluate(
    ({ data, socketIndex }) => window.__agentTest!.postToWorker({ op: "receiveRaw", data, socketIndex }),
    { data, socketIndex },
  );
}

async function closeSocket(page: Page, socketIndex?: number) {
  await page.evaluate((socketIndex) => window.__agentTest!.postToWorker({ op: "close", socketIndex }), socketIndex);
}

async function sentMessages(page: Page, type?: string) {
  const events = await page.evaluate(() => window.__agentTest?.socketEvents ?? []);
  return events
    .filter((event): event is Extract<SocketEvent, { kind: "__test_socket_sent" }> => event.kind === "__test_socket_sent")
...
test("recovers when the socket drops after a tool call but before its result", async ({ page }) => {
  ...
  await receive(page, { type: "TOOL_CALL", seq: 2, stream_id: "s", call_id: "c1", tool_name: "lookup", args: { query: "x" } });

  await expect(page.getByText("Checking")).toBeVisible();
  await expect(page.getByText("lookup")).toBeVisible();
  await expect.poll(async () => sentMessages(page, "TOOL_ACK"), { timeout: 2_000 }).toEqual([
    { type: "TOOL_ACK", call_id: "c1" },
  ]);

  await closeSocket(page);
  await expect.poll(async () => sentMessages(page, "RESUME"), { timeout: 2_000 }).toEqual([
    { type: "RESUME", last_seq: 2 },
  ]);

  await receive(page, { type: "TOOL_CALL", seq: 2, stream_id: "s", call_id: "c1", tool_name: "lookup", args: { query: "replayed" } });
...
test("handles delayed response, corrupt frames, and out-of-order replay without corrupting the DOM", async ({ page }) => {
  ...
  await expect.poll(async () => sentMessages(page, "PONG"), { timeout: 1_000 }).toEqual([
    { type: "PONG", echo: "" },
  ]);
});
